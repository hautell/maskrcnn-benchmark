...
def get_distance(x):
    ...
    distance_square = square + square.t() - (2.0 * torch.mm(x, x.t()))

    # Adding identity to make sqrt work.
    return torch.sqrt(distance_square + torch.eye(n, device=x.device))
...
class DistanceWeightedTripletSelector(TripletSelector):
    """
    For each positive pair, takes the hardest negative sample (with the greatest triplet loss value) to create a triplet
    Margin should match the margin used in triplet loss.
    negative_selection_fn should take array of loss_values for a given anchor-positive pair and all negative samples
    ...
    def get_triplets(self, embeddings, labels):
        ...
        if self.cpu:
            embeddings = embeddings.cpu()
        distance_matrix = get_distance(embeddings)
        distance_matrix = distance_matrix.cpu()
        distance_matrix = torch.max(distance_matrix, torch.ones_like(distance_matrix)*0.5)

        log_weights = ((2.0 - float(d)) * torch.log(distance_matrix)
                           - (float(d - 3) / 2) * torch.log(1.0 - 0.25 * (distance_matrix ** 2.0)))
        ...
        for label in set(labels):
            ...
            weights_ = weights_.data.numpy()

            ap_distances = distance_matrix[anchor_positives[:, 0], anchor_positives[:, 1]]
            for anchor_positive, ap_distance in zip(anchor_positives, ap_distances):
                negative_idx = np.random.choice(n, 1, p=weights_[anchor_positive[0]])[0]
